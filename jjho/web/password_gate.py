...
password. A correct password (compared in constant time in :mod:`.app`)
stores only a *marker* in Flask's signed session cookie — signed with
``SESSION_SECRET`` via itsdangerous, so the raw password is never stored in
the cookie or logged. Unset / empty ``APP_PASSWORD`` = gate OFF: local-dev
only, never expose the app with the gate off.

This module holds the stateless helpers and the in-memory per-IP failed-login
rate limiter; the routes + ``before_request`` gate live in :mod:`.app`. Mirrors
the taste-twin / todoist-points / km-tracker shared-password pattern exactly.
"""
...
class LoginRateLimiter:
    ...
    of 10 bad guesses locks that IP for ~15 minutes). Process-local — a restart
    clears it.
    """
